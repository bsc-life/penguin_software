...
list_cluster_tabs[[5]] <- cbind(list_cluster_tabs[[5]],pvalue_ratio,pvalue_degree)
double_sig <- list_cluster_tabs[[5]][intersect(which(pvalue_ratio < 0.05),which(pvalue_degree<0.05)),]
write.table(x=double_sig,"significant_proteins_cluster_5.txt",sep = "\t",row.names = F)
write.table(x=list_cluster_tabs[[5]],"full_betweness_proteins_cluster_5.txt",sep = "\t",row.names = F)
